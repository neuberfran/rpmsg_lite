...
MEMORY
    {
    FLASH (rx) : ORIGIN = (0x0 + 0x0), LENGTH = 0x31000
    RAM (wx) : ORIGIN = 0x20000000, LENGTH = (128 * 1K)
    SRAM1 : ORIGIN = 536936448, LENGTH = 65536 SRAM2 : ORIGIN = 537001984, LENGTH = 32768
...
.intList :
{
 ...
 KEEP(*(.gnu.attributes))
    ...
    SRAM1 536936448 (NOLOAD) : { __SRAM1_start = .; KEEP(*(SRAM1)) KEEP(*(SRAM1.*)) __SRAM1_end = .; } > SRAM1 __SRAM1_size = __SRAM1_end - __SRAM1_start; __SRAM1_load_start = LOADADDR(SRAM1); SRAM2 537001984 (NOLOAD) : { __SRAM2_start = .; KEEP(*(SRAM2)) KEEP(*(SRAM2.*)) __SRAM2_end = .; } > SRAM2 __SRAM2_size = __SRAM2_end - __SRAM2_start; __SRAM2_load_start = LOADADDR(SRAM2);
.last_section :
{
  LONG(0xE015E015)
} > FLASH
